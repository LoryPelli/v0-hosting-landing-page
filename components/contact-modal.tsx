...
export function ContactModal({ isOpen, onClose }: ContactModalProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex items-center justify-center p-4 ${isClosing ? "animate-out fade-out duration-300" : "animate-in fade-in duration-300"}`}
    >
      <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={handleClose} />

      <div
        className={`relative glass-card rounded-3xl p-8 max-w-md w-full shadow-2xl border-2 border-white/20 bg-slate-900/95 ${isClosing ? "animate-out zoom-out-95 duration-500" : "animate-in zoom-in-95 duration-500"}`}
      >
        ...
        <div className="relative">
          ...
          <div className="flex gap-3">
            <Button
              onClick={handleCopy}
              className="flex-1 liquid-button relative z-10 rounded-full py-6 text-base font-semibold text-white bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 overflow-hidden transition-all duration-500 hover:scale-105 hover:shadow-[0_0_40px_rgba(147,51,234,0.6)]"
              size="lg"
            >
